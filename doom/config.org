  ...
  (defun custom-flymake-popon-format-diagnostic (diagnostic)
    ...
    (with-temp-buffer
      (insert
       "* "
       (propertize
        (string-trim (string-join (split-string (flymake-diagnostic-text diagnostic '(message)) "[\n\r]") ": "))
...
(map! :leader
      :desc "Fill whole buffer"
      "b f" #'fill-whole-buffer)
#+end_src
...
#+begin_src emacs-lisp :tangle yes
(defun my/magit-worktree ()
  "Open Magit's worktree menu."
  (interactive)
  (require 'magit)
  (magit-worktree))
...
(map! :leader
      :desc "Worktrees"
      "g w" #'my/magit-worktree)
#+end_src

#+RESULTS:
: (Worktrees . my/magit-worktree)

#+begin_src emacs-lisp :tangle yes
(defun my/projectile-switch-git-worktree ()
  "Switch Projectile to a Git worktree of the current repository."
  (interactive)
         ...
         (dirs
          (delq nil
                (mapcar
                 ...
                 worktrees)))
         (choice (completing-read "Worktree: " dirs nil t)))
    (projectile-switch-project-by-name choice)))

(map! :leader
      :desc "Worktrees"
      "p w" #'my/projectile-switch-git-worktree)
#+end_src
...
(use-package! agent-shell-hq
  ...
  :config
  (add-to-list 'evil-buffer-regexps '("\\` \\*agent-shell-hq-" . emacs)))
...
(map! :leader
      ...
      :desc "Agent Shell HQ"
      "h" #'agent-shell-hq-toggle

      :desc "Peek Agent Shell HQ"
...
** Org
#+begin_src emacs-lisp :tangle yes
(setq org-default-notes-file "~/documents/orgs/gtd/inbox.org")

(setq org-directory "~/documents/org/"
      org-noter-notes-search-path "~/documents/notes/"
      org-roam-directory "~/documents/org/roam/wiki")
#+end_src

#+begin_src emacs-lisp :tangle yes
(setq journal/dir "~/documents/org/journal")
(map! :leader
      :desc "Open journal"
      "n j" #'journal/open)
#+end_src
...
#+begin_src emacs-lisp :tangle yes
(after! org
  (setq gtd/dir "~/documents/org/gtd")
  (gtd/init "~/documents/org/gtd"))
#+end_src
...
#+begin_src emacs-lisp :tangle yes
(after! projectile
  (dolist (f '("pyproject.toml" "uv.lock"))
...
#+begin_src emacs-lisp :tangle yes
(after! treemacs
  (defun my-treemacs-ignore-p (name _path)
    (member name '(".pytest_cache"
                   ".venv"
                   ".ropeproject"
                   ".mypy_cache")))

  (add-to-list 'treemacs-ignored-file-predicates
               #'my-treemacs-ignore-p))
#+end_src

#+RESULTS:
| my-treemacs-ignore-p | treemacs-ignore-pycache | treemacs--std-ignore-file-predicate |
